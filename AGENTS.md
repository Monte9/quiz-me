...
- **Cross-project skills** (`memory`, `nano-banana`, `write-spec`, `sync-to-notion`, etc.) — only available locally on Monte's laptop. Cloud agents on this repo use only the in-repo `quiz-me` skill.
...
- [`.claude/skills/quiz-me/`](.claude/skills/quiz-me/) — the in-repo skill
